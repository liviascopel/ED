...
int hash(void *elemento)
{
    char *string = (char*)elemento;
    int i;
    int total = 0;
...
void Imprime(Hash tabela, int (*Compara) (void*, void*))
{
    int i;
    int n;
    tPalavra **vetor;
...
tPalavra *Acessa(Hash tabela, void *chave, int (*hash)(void*))
{
    tPalavra *p;
    int h = hash(chave);
    for (p = tabela[h]; p != NULL; p = p->prox)
    {
        if (strcmp(p->pal, chave) == 0)
            return p;
    }
